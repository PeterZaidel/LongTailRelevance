...
//              parameters.rank_shows.put(rank, parameters.rank_shows.getOrDefault(rank, 0L) + 1);
    ...
    private String[] reduce_final_rank(MutableBoolean isNotZero) {
        ...
        String[] res = new String[CTRParameters.MAX_RANK];
        for(int r = 0; r<CTRParameters.MAX_RANK; r++)
        {
            ...
            res[r] =  Double.toString(rank_ctr);
...
//        rank_ctr = rank_ctr.substring(0, rank_ctr.length() -1);
        ...
        if(!isPositive) {
            isNotZero.setValue(isNotZero.booleanValue() || false);
            return new String[CTRParameters.MAX_RANK];
        }
...
//            }
           ...
           // out = global_ctr_out + DELIMETER + rank_ctr_out;
            ...
            outValues.add(global_ctr_out);
            for(String s : rank_ctr_out)
            {
                if(s == null || s.isEmpty())
                {
                    outValues.add(null);
            ...
            List<String> outFeatureNames =  new ArrayList<>();
            outFeatureNames.add("url_global_ctr");
            for (int i = 1; i < outValues.size(); i++) {
                outFeatureNames.add("url_rank_ctr_"+Integer.toString(i));
            }
...
//            }
            ...
            String[] outValues = {qdoc_ctr_out};
            String[] outFeatureNames = {"qdoc_ctr"};
            List<ReduceResult.IdxFeaturePair> outPairs = ReduceResult.toIdxFeaturePairs(
